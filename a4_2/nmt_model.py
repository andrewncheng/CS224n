...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn = nn.Conv1d(kernel_size=2, in_channels=embed_size, out_channels=embed_size,
                                        padding=1)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # source_padded : (src_len, b) --- embedding ---> X : (src_len, b, e)
        x = self.model_embeddings.source(source_padded)
        # need x to be of shape (b, e, src_len) for conv1d layer
        x = self.post_embed_cnn(x.permute(1, 2, 0))
        # x needs to be (b,e, src_len) --> (src_len, b, e) for LSTM
        x = x.permute(2, 0, 1)
        packed_input = pack_padded_sequence(x, torch.tensor(source_lengths).cpu().numpy(), enforce_sorted=True)
        packed_output, (ht, ct) = self.encoder(packed_input)
        output, _ = pad_packed_sequence(packed_output)

        # output : (src_len, b, 2*h) ---> enc_hiddens : (b, src_len, h*2)
        enc_hiddens = output.permute(1, 0, 2)
        # ht[0] : (2, b, h) ---> (b, h * 2)
        h_dec = ht.view(-1, self.hidden_size * 2)
        c_dec = ct.view(-1, self.hidden_size * 2)

        dec_init_state = (self.h_projection(h_dec), self.c_projection(c_dec))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        ### YOUR CODE HERE (~9 Lines)
        # enc_hiddens : (b, src_len, h*2) -- W_{attProj} : (h, h*2) ---> enc_proj : (b, src_len, h)
        enc_hiddens_proj = self.att_projection(enc_hiddens)
        # Construct Y of target sentences of shape (tgt_len, b, e) using target model embeddings
        # target_padded : (tgt_len, b) -- E --> (tgt_len, b, e)
        y = self.model_embeddings.target(target_padded)

        # use torch.chunk function to iterate over the time dimension of Y : (tgt_len, b, e)
        for chunk in torch.chunk(y, y.shape[0], dim=0):
            # chunk : (1, b, e) -- squeeze --> (b, e)
            y_t = chunk.squeeze(0)
            y_bar_t = torch.cat((y_t, o_prev), dim=1)
            dec_state, combined_output, e_t = self.step(y_bar_t, dec_state, enc_hiddens,
                                                        enc_hiddens_proj, enc_masks = enc_masks)
            combined_outputs.append(combined_output)
            o_prev = combined_output

        # combined_outputs (List) length tgt_len of tensors shaped (b,h) -- stack --> tensor : (tgt_len, b, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        output, dec_state = self.decoder(Ybar_t, (dec_state[0].unsqueeze(0), dec_state[1].unsqueeze(0)))
        dec_hidden, dec_cell = dec_state
        # enc_hiddens_proj : (b, src_len, h), dec_hidden.unsqueeze(1) : (b, 1,  h) -- dot product --> e_t : (b, src_len)
        ...
        # alpha_t : (b, src_len) / enc_hiddens : (b, src_len, 2*h)
        # a_t : (b, 2h)
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1)
        # a_t : (b, 2h) / dec_hidden : (1, b, h)
