...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn = nn.Conv1d(in_channels=embed_size, out_channels=embed_size, kernel_size=2, bias=True,
                                        padding=1)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # 1.
        # this gives dimensions (src_len, b, e)
        X = self.model_embeddings.source(source_padded)

        # 2.
        # permute dim(X): (src_len, b, e) --> (b, e, src_len)
        X = X.permute(1, 2, 0)
        # Apply the post_embed_cnn layer.
        X = self.post_embed_cnn(X)
        # Permute to go from (b, e, src_len) --> (src_len, b, e)
        X = X.permute(2, 0, 1)

        ### 3. Compute `enc_hiddens`, `last_hidden`, `last_cell` by applying the encoder to `X`.
        # Pack the padded inputs so RNN ignores the padded entries
        packed_input = pack_padded_sequence(X, torch.tensor(source_lengths))
        # Apply LSTM on packed_input --> (output, h_n, c_n)
        # --> output : (src_len,b,h*2),  h_n : (2, b, h), c_n : (2, b, h)
        packed_enc_hidden, (last_hidden, last_cell) = self.encoder(packed_input)
        enc_hiddens, input_sizes = pad_packed_sequence(packed_enc_hidden)

        # Transform dimension of enc_hidden: (src_len, b, h*2) --> (b, src_len, h*2)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)

        ### 4:  Compute `dec_init_state` = (init_decoder_hidden, init_decoder_cell):
        # `last_hidden` is a tensor shape (2, b, h). The first dimension corresponds to forwards and backwards.
        #  Concatenate the forwards and backwards tensors to obtain a tensor shape (b, 2*h).
        #  Then apply the h_projection layer to this in order to compute init_decoder_hidden to obtain h_0^{dec} in pdf
        #  Repeat with last_cell with c_projection layer to obtain c_0^{dec}
        last_hidden_concat = last_hidden.reshape(-1, self.hidden_size * 2)
        last_cell_concat = last_cell.reshape(-1, self.hidden_size * 2)
        h0_dec = self.h_projection(last_hidden_concat)
        c0_dec = self.c_projection(last_cell_concat)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # 1. enc_hiddens : (b, src_len, h*2) -- W_{attProject} -->  enc_hiddens_proj : (b, src_len, h)
        enc_hiddens_proj = self.att_projection(enc_hiddens)

        # 2. target_padded : (tgt_len, b) --> target_model_embeddings --> Y : (tgt_len, b, e)
        Y = self.model_embeddings.target(target_padded)

        # 3. Apply torch.split to iterate over the time dimension of Y
        tgt_len = target_padded.shape[0]
        # Y : (tgt_len, b, e) --> chunk --> Y_chunk : tuple of length tgt_len of tensors of size (1, b, e)
        Y_chunk = Y.chunk(chunks=tgt_len, dim=0)
        for Y_t in Y_chunk:
            # chunk : (1, b, e) --> squeeze --> (b, e)
            Y_t = Y_t.squeeze(0)
            # (Y_t, o_prev) --> concat --> (b, e + h) where e = embedding dim and h = hidden dim
            Ybar_t = torch.concat((Y_t, o_prev), dim=1)
            # dec_state (tuple (Tensor, Tensor)), Tensor : (b,h), combined_outputs : (b,h)
            dec_state, combined_output, _ = self.step(Ybar_t=Ybar_t, dec_state=dec_state, enc_hiddens=enc_hiddens,
                                                       enc_hiddens_proj=enc_hiddens_proj, enc_masks=enc_masks)
            # append o_t to combined_outputs?
            combined_outputs.append(combined_output)

            # updated o_prev to new o_t
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # 1. LSTM input is of the form (input, (h_0, c_0)) where c_0, h_0 : (1, b, h)
        # However, as input dec_state (Tensor, Tensor) where Tensor : (b,h)
        Ybar_t = Ybar_t.unsqueeze(0)
        output, (dec_hidden, dec_cell) = self.decoder(Ybar_t, (dec_state[0].unsqueeze(0), dec_state[1].unsqueeze(0)))
        # dec_hidden : (1, b, h) --> transpose --> (b, h, 1)
        # enc_hiddens_proj : (b, src_len, h)
        # dec_hidden, enc_hiddens_proj --- bmm ---> (b, src_len, 1) --> squeeze --> (b, src_len)
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.permute(1, 2, 0)).squeeze(2)
        ...
        # e_t : (b, src_len) --> softmax ---> alpha_t : (b, src_len)
        alpha_t = torch.nn.functional.softmax(e_t, dim=1)
        # alpha_t -- unsqueeze(alpha_t, 1) ---> alpha_t : (b, 1, src_len)
        alpha_t = torch.unsqueeze(alpha_t, 1)
        # alpha_t : (b, 1, src_len), enc_hiddens : (b, src_len, h * 2) -- bmm --> (b, 1, h*2)
        a_t = torch.bmm(alpha_t, enc_hiddens)
        a_t = a_t.squeeze(1)

        # a_t : (b, h * 2); dec_h : (1, b, h)
        # dec_hidden : (1, b, h) -- squeeze --> (b, h)
        dec_hidden = dec_hidden.squeeze(0)
        # a_t, dec_h -- cat --> u_t = [a_t, dec_h] : (b, 3 * h)
        u_t = torch.cat((a_t, dec_hidden), dim=1)

        # u_t : (b, 3 * h) --> W_u --> v_t : (b, h)
        v_t = self.combined_output_projection(u_t)
        # v_t : (b,h)  --- dropout --> O_t : (b,h)
