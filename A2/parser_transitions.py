...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S" and self.buffer:
            ...
            # print(f"stack = {self.stack}, buffer = {self.buffer}")
            self.stack.append(self.buffer.pop(0))
        elif transition == "RA" and len(self.stack) > 1:
            # define head as the stem of the arrow (in this case last element); head will be RHS of tuple
            (head, tail) = (self.stack[-2], self.stack[-1])
            self.dependencies.append((head, tail))
            # print(f"transition = {transition}")
            # print(f"stack = {self.stack}, buffer = {self.buffer}")
            # print("dependency", self.dependencies)
            self.stack.pop()
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        batched_parses = unfinished_parses[:batch_size]
        pred_transitions = model.predict(batched_parses)
        for partial_parse, transition in zip(batched_parses, pred_transitions):
    ...
    print("partial_parses====", partial_parses)
